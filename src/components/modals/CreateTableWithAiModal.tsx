...
export const CreateTableWithAiModal: React.FC<CreateTableWithAiModalProps> = ({
  ...
  const handleSubmit = async (event?: React.SyntheticEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create table with AI. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="bg-modal-backdrop relative" role="dialog" aria-modal="true" tabIndex={-1}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-xl font-semibold text-primary truncate">Create Table with AI</h2>
              <p className="text-sm text-secondary truncate">Describe the table you want to generate</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form id="create-table-with-ai-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto overflow-x-hidden min-h-0">
          <div className="p-4 space-y-4">
            <MultiLineText
              ...
              placeholder="Example: Create a project table with task name, assignee, status, priority, due date, and notes."
              rows={8}
              isBorder={true}
              required
            />
