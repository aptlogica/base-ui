...
export interface PreviewTableRelation {
  type: string;
  source_table: string;
  target_table: string;
}
...
export interface PreviewTableData {
  ...
  relations: PreviewTableRelation[];
}

interface PreviewTableModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: PreviewTableData | Record<string, any> | { data?: Record<string, any> } | null;
  onApply?: (data: PreviewTableData, sampleData: boolean) => Promise<void> | void;
}
...
const normalizePreviewTableData = (input: PreviewTableModalProps['data']): PreviewTableData => {
  const payload = (() => {
    if (!input || typeof input !== 'object') {
      return {};
    }

    if ('data' in input && input.data && typeof input.data === 'object' && !Array.isArray(input.data)) {
      return input.data;
    }
  ...
  const tables = Array.isArray((payload as Record<string, any>).tables)
    ? (payload as Record<string, any>).tables.map((table: any) => ({
        name: String(table?.name || table?.title || 'Untitled Table'),
  ...
  const relations = Array.isArray((payload as Record<string, any>).relations)
    ? (payload as Record<string, any>).relations.map((relation: any) => ({
  ...
  const fallbackName = String((payload as Record<string, any>).table_name || (payload as Record<string, any>).title || (payload as Record<string, any>).name || 'Untitled Table');
  return {
    tables: [{
      name: fallbackName,
      fields: [],
...
export const PreviewTableModal: React.FC<PreviewTableModalProps> = ({
  isOpen,
  onClose,
  data,
  onApply,
}) => {
  const [draftData, setDraftData] = useState<PreviewTableData | null>(null);
  ...
  useEffect(() => {
    ...
    setDraftData(data ? createDraftData(normalizePreviewTableData(data)) : null);
    setSelectedTableIndex(0);
    setIncludeSampleData(false);
    setIsApplying(false);
  }, [data, isOpen]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      console.error('Failed to apply table preview schema:', err);
    } finally {
      setIsApplying(false);
    }
  };
        ...
        <div className="flex items-center justify-between p-4 border-b flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-xl font-semibold text-primary truncate">Preview Table</h2>
              <p className="text-sm text-secondary truncate">
                Review the generated table schema before applying it
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-500">
                    This name will be used for the generated table.
                  </p>
                </div>
              </div>
