...
const BaseRoleManagement: React.FC<{
  ...
  baseRoleOptions: Array<{ label: string; value: string }>;
  onRoleChange: (baseId: string) => (value: string) => void;
  onRemoveAccess: (baseId: string, baseName: string) => () => Promise<void>;
  isPending: boolean;
}> = React.memo(({ userBases, baseRoles, baseRoleOptions, onRoleChange, onRemoveAccess, isPending }) => {
            ...
            <div key={baseId} className="flex items-center gap-4 justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center gap-3">
                <AdvancedDropdown
                  ...
                  onChange={(value) => {
                    const role = (Array.isArray(value) ? value[0] : value) as string;
                    onRoleChange(baseId)(role);
                  }}
                ...
                <button
                  type="button"
                  onClick={onRemoveAccess(baseId, baseName)}
                  disabled={isPending}
...
BaseRoleManagement.displayName = 'BaseRoleManagement';

// Component for workspace role management - extracted to reduce complexity
const WorkspaceRoleManagement: React.FC<{
...
}> = React.memo(({ workspaceName, workspaceInitials, selectedRole, roleOptions, onRoleChange, onRemoveAccess, isPending, isLoadingUserAccess }) => {
  return (
    <div className="flex items-center gap-4 justify-between">
      ...
      <div className="flex-1 max-w-xs flex items-center gap-3">
        <AdvancedDropdown
          ...
          placeholder="Select a role"
          disabled={isPending || isLoadingUserAccess}
        />
...
WorkspaceRoleManagement.displayName = 'WorkspaceRoleManagement';

// Component for edit mode UI - extracted to reduce cognitive complexity
const EditModeUI: React.FC<{
  userRolesAndAccessData: any;
  workspaceId: string;
  selectedRole: string;
  baseRoles: Record<string, string>;
  roleOptions: Array<{ label: string; value: string }>;
  baseRoleOptions: Array<{ label: string; value: string }>;
  onRoleChange: (role: string) => void;
  onRemoveWorkspaceAccess: (workspaceName: string) => () => Promise<void>;
  onBaseRoleChange: (baseId: string) => (value: string) => void;
  onRemoveBaseAccess: (baseId: string, baseName: string) => () => Promise<void>;
  isPending: boolean;
  isLoadingUserAccess: boolean;
}> = React.memo(({ 
...
EditModeUI.displayName = 'EditModeUI';
...
export const AssignUserToWorkspaceModal: React.FC<AssignUserToWorkspaceModalProps> = ({
  ...
  const extractWorkspaceRole = React.useCallback((workspaceAccess: string): string => {
    if (workspaceAccess && workspaceAccess !== '') {
      return workspaceAccess === 'maintainer' ? 'maintainer' : 'workspace-read';
    }
    return 'base-member'; // Default for base-level access
  }, []);

  // Helper function to build base roles map from API response
  const buildBaseRolesMap = React.useCallback((bases: any[]): Record<string, string> => {
  ...
  // Helper function to initialize edit mode data
  const initializeEditModeData = React.useCallback(() => {
    ...
    const workspaceAccess = currentWorkspace.access || '';
    const role = extractWorkspaceRole(workspaceAccess);
    setSelectedRole(role);
    setSelectedUserIds([memberToEdit!]);

    if (currentWorkspace.bases && Array.isArray(currentWorkspace.bases)) {
      const baseRolesMap = buildBaseRolesMap(currentWorkspace.bases);
      setBaseRoles(baseRolesMap);
    } else {
      setBaseRoles({});
    }
  }, [userRolesAndAccessData, workspaceId, memberToEdit, extractWorkspaceRole, buildBaseRolesMap]);
  ...
  useEffect(() => {
    ...
    if (editMode && memberToEdit) {
      initializeEditModeData();
    } else {
      initializeAddModeData();
    }
  }, [isOpen, editMode, memberToEdit, initializeEditModeData, initializeAddModeData]);
  ...
  // Helper function to validate assign user input
  const validateAssignUserInput = (): boolean => {
  ...
  // Helper function to build membership request
  const buildMembershipRequest = (): {
    workspace_id: string;
    role: string;
    bases: Array<{ base_id: string; role: string }>;
  } => {
  ...
  const buildSuccessMessage = (userCount: number, isEdit: boolean): string => {
    if (isEdit) {
      return 'Member updated successfully';
    }
    if (userCount === 1) {
      return 'User assigned to workspace successfully';
    }
    const userWord = userCount > 1 ? 'users' : 'user';
    return `${userCount} ${userWord} assigned to workspace successfully`;
  };

  // Helper function to build failure messages
  const buildFailureMessages = (failures: any[]): string[] => {
  ...
  // Helper function to handle bulk add response
  const handleBulkAddResponse = (response: any): boolean => {
    if (response?.success_count !== undefined && response?.failure_count !== undefined) {
      ...
      if (successCount > 0 && failureCount === 0) {
        const successMessage = buildSuccessMessage(successCount, editMode);
        toast.success(successMessage);
        return true;
      }

      if (successCount > 0 && failureCount > 0) {
        const failureMessages = buildFailureMessages(response.failures || []);
        toast.error(
    ...
    const successMessage = buildSuccessMessage(selectedUserIds.length, editMode);
    toast.success(successMessage);
    return true;
  };
  ...
  const handleAssignUser = async () => {
    if (!validateAssignUserInput()) {
      return;
    }

    try {
      const membership = buildMembershipRequest();
      ...
      const shouldClose = handleBulkAddResponse(response);
      if (!shouldClose) {
        return;
      }
  ...
  // Handler for base role change - extracted to reduce nesting
  const handleBaseRoleChange = React.useCallback((baseId: string) => {
    return (value: string) => {
      setBaseRoles(prev => ({ ...prev, [baseId]: value }));
    };
  }, []);

  // Handler for removing base access - extracted to reduce nesting
  const handleRemoveBaseAccess = React.useCallback((baseId: string, baseName: string) => {
    return async () => {
