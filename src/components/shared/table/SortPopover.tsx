...
export const SortPopover: React.FC<SortPopoverProps> = ({ columns, sorts, onChange }) => {
  ...
  const validSortsCount = useMemo(() => {
    return displaySorts.filter(sort => sort.column && sort.column.trim()).length;
  }, [displaySorts]);
  ...
  const fieldsToExcludeFromSort = ['links', 'lookup', 'rollup', 'attachment', 'json'];
  const availableColumns = columns.filter(col => {
    const uidt = String(col.uidt || col.type || '').toLowerCase();
    // Exclude non-sortable field types
    if (fieldsToExcludeFromSort.includes(uidt)) return false;
    ...
    if (['createdTime', 'lastModifiedTime', 'createdBy', 'lastModifiedBy'].includes(uidt)) return false;
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (isPendingSort) {
                                ...
                                const pendingSort = pendingSorts[pendingIdx];
