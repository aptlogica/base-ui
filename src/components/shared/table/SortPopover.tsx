...
export const SortPopover: React.FC<SortPopoverProps> = ({ columns, sorts, onChange }) => {
  ...
  const handleFieldSelect = useCallback((idx: number, key: string) => {
    ...
    setFieldDropdownOpen(null);
  }, [pendingSorts, sorts, onChange]);

  const handleDirSelect = useCallback((idx: number, dir: 'asc' | 'desc') => {
    const isPendingSort = idx >= sorts.length;

    if (isPendingSort) {
      const pendingIdx = idx - sorts.length;
      const newPendingSorts = [...pendingSorts];
      newPendingSorts[pendingIdx].direction = dir;
      setPendingSorts(newPendingSorts);
    } else {
      const newSorts = [...sorts];
      newSorts[idx].direction = dir;
      onChange(filterValidSorts(newSorts));
    }
    setDirDropdownOpen(null);
  }, [pendingSorts, sorts, onChange]);
  ...
  const handleAddSort = useCallback(() => {
    ...
    setDirDropdownOpen(null);
  }, [displaySorts.length, hasUnusedColumns]);

  const getColumnOptionsForSort = useCallback((currentKey: string) => {
  ...
  const getDropdownOptions = useCallback((columnOptions: BaseColumn[]): FieldSelectOption[] => {
    return columnOptions.map(c => ({
      key: getColumnKey(c),
      title: c.title,
      uidt: c.uidt,
      type: c.type,
  ...
  const getSortRowKey = (currentKey: string, idx: number) => currentKey || `sort-row-${idx}`;

  const renderSortRow = useCallback((sort: SortItem, idx: number) => {
    ...
    const columnOptions = getColumnOptionsForSort(currentKey);
    const dropdownOptions = getDropdownOptions(columnOptions);
    const rowKey = getSortRowKey(currentKey, idx);
    return (
      <div key={rowKey} className="flex items-center gap-2 mb-2">
        ...
        <div className="relative flex-1">
          <FieldSelectDropdown
            ...
            onSelect={(key) => handleFieldSelect(idx, key)}
              ...
              <div className="absolute z-50 mt-1 p-2 space-y-1 left-0 w-32 text-primary bg-background border rounded-xl shadow-lg max-h-32 overflow-y-auto">
                {['asc', 'desc'].map((dir) => (
                  <button
                    ...
                    onClick={() => handleDirSelect(idx, dir as 'asc' | 'desc')}
                    type="button"
                  >
    ...
    dirDropdownOpen,
    fieldDropdownOpen,
    getColumnOptionsForSort,
    getDropdownOptions,
    handleDirSelect,
    handleFieldDropdownToggle,
    handleFieldSelect,
    handleRemoveSort,
  ]);
