...
export const renderAttachmentInline = (items: any[]): React.ReactNode => {
  const attachments = extractAttachmentItems(items);
  ...
  return (
    <div className="flex items-center gap-1 max-w-full overflow-hidden">
      {visibleAttachments.map((file, index) => {
        ...
        return (
          <div
            key={`att-inline-${index}-${file.name}`}
...
export const renderAttachmentDropdown = (items: any[]): React.ReactNode => {
  const attachments = extractAttachmentItems(items).slice(0, 7);
  if (attachments.length === 0) return null;
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[320px] overflow-auto">
      {attachments.map((file, index) => {
        const image = isImageAttachment(file);
        return (
          <div key={`att-${index}-${file.name}`} className="bg-card border border-gray-200 rounded-lg overflow-hidden">
      ...
      {open && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative bg-card border rounded-xl shadow-xl w-full max-w-5xl h-[85vh] p-5 flex flex-col z-10">
            ...
            <div className="flex-1 overflow-auto min-h-[360px] bg-background border rounded-xl p-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {attachments.map((file, index) => {
                  const image = isImageAttachment(file);
                  return (
                    <div key={`modal-att-${index}-${file.name}`} className="bg-card border border-gray-200 rounded-lg overflow-hidden">
