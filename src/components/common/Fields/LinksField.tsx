...
import { useTable, useInsertRelationData, useGetRecordsByPagination } from '../../../hooks/useApi';
...
export const LinksField: React.FC<LinksFieldProps> = ({
    ...
    // Use allRecords instead of tableData records
        ...
        if (records.length > 0) {
            ...
            for (const id of valueIds) {
                ...
                if (foundRecord) {
                    ...
                    const foundByString = records.find(r => r.id.toString() === id.toString());
                    if (foundByString) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') {
                ...
                setDropdownPosition(null);
                setFocusedRecordIndex(-1);
            } else if (e.key === 'Enter' && focusedRecordIndex >= 0 && filteredRecords[focusedRecordIndex]) {
                e.preventDefault();
                handleSelectRecord(filteredRecords[focusedRecordIndex]);
    ...
    }, [isOpen, focusedRecordIndex, filteredRecords, handleSelectRecord]);

    // Scroll focused record into view
    useEffect(() => {
        if (focusedRecordIndex >= 0 && recordsListRef.current) {
    ...
    return (
        <div className="relative w-full min-w-0">
            ...
            {isOpen && dropdownPosition && createPortal(
                <div
                    ref={dropdownRef}
                    className="fixed z-50 bg-card border rounded-lg shadow-xl max-h-96 w-96 overflow-hidden flex flex-col"
                    style={{
                ...
                >
                    ...
                    <div className="p-4 border-b bg-gray-50 flex-shrink-0">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-10 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-transparent bg-background"
                            />
                            {searchTerm && (
                                <button
                                    type='button'
                    ...
                    >
                        ...
                        {/* Load More button */}
                        {hasMore && !isLoadingMore && !debouncedSearchTerm.trim() && filteredRecords.length > 0 && (
                            <div className="p-3 text-center border-t">
                                ...
                                >
                                    Load More ({allRecords.length} loaded)
                                </button>
                            </div>
                        )}
                        {/* End of list indicator */}
                        {!hasMore && filteredRecords.length > 0 && !debouncedSearchTerm.trim() && (
                            <div className="p-2 text-center text-xs text-gray-400 border-t">
                                All records loaded ({allRecords.length} total)
                            </div>
                        )}
                    </div>
